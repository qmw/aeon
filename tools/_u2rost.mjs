...
await p.evaluate((v) => { window.__NOFLAG = v[0]; window.__ISO = v[1]; window.__EL = v[2]; }, [!!process.env.NOFLAG, !!process.env.ISO, +(process.env.EL||38)]);
const installFn = ([dist]) => {
  ...
  const D = +dist;
  const e = (+(window.__EL||38)) * Math.PI / 180;
...
for (let i = 0; i < 40; i++) {
  ...
  await p.evaluate((v) => { window.__NOFLAG = v[0]; window.__ISO = v[1]; window.__EL = v[2]; }, [!!process.env.NOFLAG, !!process.env.ISO, +(process.env.EL||38)]).catch(() => {});
