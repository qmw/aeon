...
const arms = (sleeve = C.cloak) => [
  D(P('sph', 3, C.leatherD, M_LEATH, 0, 0.012, 0.004, 0.148, 0.112, 0.152)),
  D(P('sph', 4, C.leatherD, M_LEATH, 0, 0.012, 0.004, 0.148, 0.112, 0.152)),
  D(P('limb', 3, sleeve, M_CLOTH, 0.034, -0.118, 0, 0.092, 0.128, 0.094, 0, 0, 0.32)),
...
export class Units {
  ...
  // ------------------------------------------------------------------ frame
  update(dt) {
    // TAKE THE INTEGRATOR'S dt. This module used to overwrite it with wall-clock elapsed time
    // (clamped at 0.2 s), which is the one thing main.js's fixed 1/60 step exists to prevent:
    // at this box's ~0.57 fps every figure walked up to a fifth of a second of idle between
    // the two frames post.js's TAA blends, so the cast arrived ghosted — soft pauldrons, a
    // doubled helmet rim, a sword edge with no line on it — and at a pose that was different
    // on every capture. Everything else in the frame already animates on the fixed step; the
    // units were the only objects in it moving twelve times too far per frame, and they are
    // the objects the eye lands on first. Nothing here needs real time: dt only advances idle
    // phase, smoke and the plate fade.
  ...
  portrait(type, color = 0x4fa8ff, px = 192) {
    ...
    // ...and FIT THE SHIELD, which the plate was cutting in half. The board hangs off the near
    // shoulder, so it sits ~20% closer to the lens than the aim plane and magnifies by that
    // much: at span 0.70 its rim projected 0.382 against a 0.308 half-frame and the bust shipped
    // with a brown wedge jammed into the right edge, boss and all. A hair more span, the aim
    // biased toward the shield side and dropped a shoulder's worth, and the whole round board
    // is inside the plate with air around it — which is also what centres the figure, since the
    // sword is the only thing that wants the left half.
    const foot = !(def.mounted || def.boat || def.wheels);
    const span = h * (def.mounted ? 0.86 : def.boat || def.wheels ? 1.15 : 0.78);
    const d = span / (2 * Math.tan(13 * Math.PI / 180));
    const cam = new THREE.PerspectiveCamera(26, 1, 0.05, 40);
    const aim = new THREE.Vector3(
      THREE.MathUtils.clamp((bb.min.x + bb.max.x) * 0.5, -h * 0.09, h * 0.09) + (foot ? h * 0.06 : 0),
      def.boat || def.wheels ? (bb.min.y + bb.max.y) * 0.5 : hy - h * (foot ? 0.145 : 0.07), 0);
